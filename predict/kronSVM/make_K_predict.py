...
def get_list_couples_predict_drug(dbid, DB):

    dict_ind2prot = DB.proteins.dict_ind2prot

    list_couples_predict = []
    for key in range(DB.proteins.nb):
        list_couples_predict.append((dict_ind2prot[key],dbid))

    return list_couples_predict

def make_K_predict_drug(dbid, DB, kernels, list_couples_train):

    # get the preprocessed DBdatabase 
    dict_mol2ind = DB.drugs.dict_mol2ind
    dict_prot2ind = DB.proteins.dict_prot2ind

    # couples in train
    nb_couples_train = len(list_couples_train)

    # couples for prediction
    list_couples_predict = get_list_couples_predict_drug(dbid,DB)
...
def make_K_predict_new_drug(dbid, preprocessed_DB, kernels, list_couples_train):
